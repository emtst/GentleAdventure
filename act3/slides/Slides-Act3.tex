...
\titlegraphic{%
  \begin{picture}(382,0)
    {
      \put(140,10){\includegraphics[width=2.4cm]{imperial_logo.pdf}}
      \put(220,00){\includegraphics[width=2.0cm]{Uok_Logo_RGB294.jpg}}
      \put(300,00){\includegraphics[width=2.4cm]{logo-large-london.jpg}}
      \put(-10,15){
    \begin{footnotesize}
      $
      \begin{array}{l}
        \text{DisCoTec 2022}\\ \text{Lucca, 13-06-2022}
      \end{array}
      $
    \end{footnotesize}
}
...
\title{Coq-Metatheory for Smol-Zooid}
\subtitle{A Gentle Adventure Mechanising Message Passing Concurrency Systems, Act 3}

\author{\scriptsize David Castro-Perez, Francisco Ferreira, \textbf{Lorenzo Gheri}, Martin Vassor, and Nobuko Yoshida}
\institute{}
\date{}
%\date{}
...
\begin{tikzpicture}
    ...
    every node/.style={minimum width=1.8cm}]+
		\node [] (G) at (0,0) {\footnotesize $\G$};
		\node [] (L1) at (-1.5,.8 ) {\footnotesize $\dlt{\lT_1}$};
		\node [] (L2) at (-.50,.8 ) {\footnotesize $\dlt{\lT_2}$};
		\node [] (LL) at (.50 ,.8 ) {\footnotesize $\dlt{\dots}$};
		\node [] (Ln) at ( 1.5,.8 ) {\footnotesize $\dlt{\lT_n}$};
		\node [] (P1) at (-1.5,1.6) {\footnotesize $\dproc{\proc_1}$};
		\node [] (P2) at (-.50,1.6) {\footnotesize $\dproc{\proc_2}$};
		\node [] (PP) at ( .50,1.6) {\footnotesize $\dots$};
		\node [] (Pn) at ( 1.5,1.6) {\footnotesize $\dproc{\proc_n}$};
		\draw [->] (G.south) to (L1);
		\draw [->] (G.south) to (L2);
		...
		\draw [->] (Ln) to (Pn);
	    \node  at (-2,0.4) {\footnotesize projection ($\upharpoonright$)};
	    \node  at (-2.4,1.2) {\footnotesize typing ($\ofLt$)};
\end{tikzpicture}
...
\end{figure}
\begin{itemize}
\item unravelling preserves projection; focus on coinduction ($1^{\text{st}}$ square)
\item trace equivalence; focus on soundness  ($2^{\text{nd}}$ square)
\end{itemize}


\ \\ %\ \\
%Embark on our Gentle Adventure!!!
\url{https://github.com/emtst/GentleAdventure/act3}
\end{small}
\end{frame}
...
\comsgn \p\q \dcog{ (\tS).} \coG \arrow[rr, "!\p\q\tS "]\arrow[dd,"\upharpoonright_{\p}"]
\& \&
\comsgs \p\q {\ } \dcog{ (\tS).} {\coG}\arrow[rrdd, "\upharpoonright_{\p}"]
\& \&
\\
\& \& \& \&\\
\colsnd \q \dcol{ (\tS).} \colT \arrow[rrrr, "!\p\q\tS"]\& \& \& \&\colT
...
\comsgn \p\q \dcog{ (\tS).} {\coG} \arrow[rr, "!\p\q\tS "]\arrow[dd,"\upharpoonright_{\q}"]
\& \&
\comsgs \p\q {\ } \dcog{ (\tS).} {\coG}\arrow[lldd, "\upharpoonright_{\q}"]
\\
\& \& \\
\colrcv \p \dcol{ (\tS).} \colT\dcol{'} \&\&
\end{tikzcd}
\]
...
\begin{frame}{You Suffer...}

  \begin{small}
    \begin{itemize}
    \item Formal proofs are not easy. %! %(But useful and fun!)
    \item Proof design is the key.
    \item Proof techniques are to be taken seriously: (co)induction, functions VS relations, treatment of bindings...
    \end{itemize}
  ...
  %% \begin{small}
  %%   \textcolor{violet}{Check out our material!}
  %% \end{small}

  \begin{footnotesize}
    \begin{itemize}

    \item[$\rightarrow$] D. Castro-Perez, F. Ferreira, L. Gheri, and N. Yoshida.  \emph{''Zooid: a DSL for certified multiparty computation: from mechanised metatheory to certified multiparty processes''}. PLDI 2021. \\
      DOI: \url{https://doi.org/10.1145/3453483.3454041} \\
  website: \url{http://mrg.doc.ic.ac.uk/publications/zooid-paper/}


\item[$\rightarrow$]This tutorial is available at \url{https://github.com/emtst/GentleAdventure}
  \end{itemize}
\end{footnotesize}
...
\begin{frame}{... but Why?}
  ...
  \begin{footnotesize}
    Formal proofs are not easy\visible<2->{\textcolor{violet}{, but useful and fun!}\\
    As witnessed, e.g., by the influential POPLmark Challenge\footnote{Aydemir et al. \emph{``Mechanized Metatheory for the
Masses: The POPLmark challenge.''} 2005}...}
  \end{footnotesize}


\begin{center}
  \visible<3->{\textcolor{violet}{Towards a \bf{Concurrent Calculi Formalisation Benchmark}}}\\
  %\visible<4->{\textcolor{violet}{\small For the mechanisation of process calculi and behavioural types}}
\end{center}

\visible<3->{\begin{small}
\begin{block}{Challenge problems:}

    \begin{itemize}
    \item name passing and scope extrusion
    \item linearity and behavioural type systems
    \item coinduction and reasoning about process algebras
    \end{itemize}
\end{block}
\end{small}}
...
\begin{frame}{The Future is Unwritten... But Sketched!}
%% \vspace{-2mm}
%%   \begin{small}
%%     \begin{itemize}
%%     \item Adding Features for Reasoning about Processes
%%     \item Certifying Existing Systems (e.g., integration with $\nu$Scr)
...
%% \end{footnotesize}
  ...
  \begin{columns}
    \begin{column}{0.4\textwidth}
      \begin{center}
        %\vspace{-5mm}
        \includegraphics[width=\textwidth]{benchmark.jpeg}
      \end{center}
    \end{column}
    \begin{column}{0.6\textwidth}
      \begin{footnotesize}
        \hspace*{-5cm}\begin{itemize}
        \item Concurrent Benchmark website: \url{https://concurrentbenchmark.github.io/}
        %\item[$\rightarrow$] Fill in the form and join us!
        %\item[$\rightarrow$] First goal: write a community paper.
        \item This tutorial:\\ \url{https://github.com/emtst/GentleAdventure}
        \end{itemize}
      \end{footnotesize}
      %% \ \\
      %\textcolor{violet}{Join us!}
      %\\
      \ \\ \ \\
      \visible<2>{
        \begin{center}
          \textcolor{violet}{\bf{THANK YOU!}}
          \end{center}
      }
    \end{column}
  \end{columns}
